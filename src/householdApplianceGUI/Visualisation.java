...
public class Visualisation extends TimerTask{
	...
	private ApplianceOFFHeaterR3A9Thread applianceOFFHeaterR3A9Thread = null;
	private ApplianceOKHeaterR3A9Thread applianceOKHeaterR3A9Thread = null;
	private AppliancePowerLightR3A10Thread lightR3A10Thread = null;
	private ApplianceONLightR3A10Thread applianceONLightR3A10Thread = null;
	private ApplianceOFFLightR3A10Thread applianceOFFLightR3A10Thread = null;
	private ApplianceOKLightR3A10Thread applianceOKLightR3A10Thread = null;
	private ApplianceONLightR4A12Thread applianceONLightR4A12Thread = null;
	private ApplianceOFFLightR4A12Thread applianceOFFLightR4A12Thread = null;
	private ApplianceOKLightR4A12Thread applianceOKLightR4A12Thread = null;
	...
	private JToggleButton fridgeR1A1Button, dishwasherR1A2Button, dryerR1A3Button, stoveR1A4Button, ovenR1A5Button, TVR2A6Button, powerplugR2A7Button, heaterR2A8Button, heaterR3A9Button, lightR3A10Button, powerplugR3A11Button, lightR4A12Button, fanR4A13Button, waterHeaterR4A14Button, humanPresenceR3ToggleButton, humanPresenceR4ToggleButton = null;
	...
	public void run()
	{
		...
		if(humanPresenceR3ToggleButton.isSelected())
		{
			HumanPresenceR3("PRESENT");
		}
		else
		{
			HumanPresenceR3("ABSENT"); 
		} 
		
		if(humanPresenceR4ToggleButton.isSelected())
		{
			HumanPresenceR4("PRESENT");
		}
		else
		{
			HumanPresenceR4("ABSENT"); 
		} 
	    cal.add(Calendar.MINUTE,1);
	}
	
	/**
	 * Human Presence/Absence for Room R4
	 */
	private void HumanPresenceR4(String presentAbsent)
	{
		if (presentAbsent.equalsIgnoreCase("PRESENT")) 
		{
			humanPresenceR4ToggleButton.setIcon(new ImageIcon("D:\\Projects\\ApplianceModelingAndSimulation\\images\\Human_PRESENT.jpg"));
			Socket skt = null;
			PrintWriter out = null;
			BufferedReader in = null;	
			try {
				skt = new Socket(InetAddress.getByName("127.0.0.1"), 44474);
				in = new BufferedReader(new InputStreamReader(skt.getInputStream()));
				out = new PrintWriter(skt.getOutputStream(),true);
			    out.println(true);
			    in.close();out.close();skt.close();
			    applianceONLightR4A12Thread.join(3);
		...
		else if(presentAbsent.equalsIgnoreCase("ABSENT"))
		{
			humanPresenceR4ToggleButton.setIcon(new ImageIcon("D:\\Projects\\ApplianceModelingAndSimulation\\images\\Human_ABSENT.jpg"));	
			Socket skt = null;
			PrintWriter out = null;
			BufferedReader in = null;	
			try {
				skt = new Socket(InetAddress.getByName("127.0.0.1"), 44475);
				in = new BufferedReader(new InputStreamReader(skt.getInputStream()));
				out = new PrintWriter(skt.getOutputStream(),true);
			    out.println(true);
			    in.close();out.close();skt.close();
			    applianceOFFLightR4A12Thread.join(3);
			} catch (UnknownHostException e1) {
				// TODO Auto-generated catch block 
				e1.printStackTrace();
			} catch (IOException e1) {
	...
	/**
	 * Human Presence/Absence for Room R3
	 */
	private void HumanPresenceR3(String presentAbsent)
	{
		if (presentAbsent.equalsIgnoreCase("PRESENT")) 
		{
			humanPresenceR3ToggleButton.setIcon(new ImageIcon("D:\\Projects\\ApplianceModelingAndSimulation\\images\\Human_PRESENT.jpg"));
			Socket skt = null;
			PrintWriter out = null;
			BufferedReader in = null;	
			try {
				skt = new Socket(InetAddress.getByName("127.0.0.1"), 44472);
				in = new BufferedReader(new InputStreamReader(skt.getInputStream()));
				out = new PrintWriter(skt.getOutputStream(),true);
			    out.println(true);
			    in.close();out.close();skt.close();
			    applianceONLightR3A10Thread.join(3);
			} catch (UnknownHostException e1) {
				// TODO Auto-generated catch block 
				e1.printStackTrace();
			} catch (IOException e1) {
		...
		{
			humanPresenceR3ToggleButton.setIcon(new ImageIcon("D:\\Projects\\ApplianceModelingAndSimulation\\images\\Human_ABSENT.jpg"));	
			Socket skt = null;
			PrintWriter out = null;
			BufferedReader in = null;	
			try {
				skt = new Socket(InetAddress.getByName("127.0.0.1"), 44473);
				in = new BufferedReader(new InputStreamReader(skt.getInputStream()));
				out = new PrintWriter(skt.getOutputStream(),true);
			    out.println(true);
			    in.close();out.close();skt.close();
			    applianceOFFHeaterR3A9Thread.join(3);
			    applianceOFFLightR3A10Thread.join(3);
			} catch (UnknownHostException e1) {
				// TODO Auto-generated catch block 
				e1.printStackTrace();
			} catch (IOException e1) {
	...
	public static void main(String[] args) {
		...
		timer.schedule(window = new Visualisation(), 0, 1000);//schedule to execute very 1000 ms which is 1 second
	...
	private void initialize() {
		...
		applianceOFFHeaterR3A9Thread = new ApplianceOFFHeaterR3A9Thread();
		applianceOKHeaterR3A9Thread = new ApplianceOKHeaterR3A9Thread();
		lightR3A10Thread = new AppliancePowerLightR3A10Thread();
		System.out.println("Light R3 A10 Thread created");		
		applianceONLightR3A10Thread = new ApplianceONLightR3A10Thread();
		applianceOFFLightR3A10Thread = new ApplianceOFFLightR3A10Thread();
		applianceOKLightR3A10Thread = new ApplianceOKLightR3A10Thread();
		...
		applianceONLightR4A12Thread = new ApplianceONLightR4A12Thread();
		applianceOFFLightR4A12Thread = new ApplianceOFFLightR4A12Thread();
		applianceOKLightR4A12Thread = new ApplianceOKLightR4A12Thread();
		...
		applianceOFFHeaterR3A9Thread.start();
		applianceOKHeaterR3A9Thread.start();
		
		lightR3A10Thread.start();
		applianceOFFLightR3A10Thread.start();
		applianceONLightR3A10Thread.start();
		applianceOKLightR3A10Thread.start();
		
		powerplugsR3A11Thread.start();
		lightR4A12Thread.start();
		applianceOFFLightR4A12Thread.start();
		applianceONLightR4A12Thread.start();
		applianceOKLightR4A12Thread.start();
		...
		BedroomAggregatedLabel.setBounds(1075, 74, 116, 14);
		...
		humanPresenceR3ToggleButton = new JToggleButton("");
		humanPresenceR3ToggleButton.setIcon(new ImageIcon("D:\\Projects\\ApplianceModelingAndSimulation\\images\\Human_ABSENT.jpg"));
		humanPresenceR3ToggleButton.setBounds(1219, 312, 46, 47);
		frame.getContentPane().add(humanPresenceR3ToggleButton);
		
		humanPresenceR4ToggleButton = new JToggleButton("");
		humanPresenceR4ToggleButton.setIcon(new ImageIcon("D:\\Projects\\ApplianceModelingAndSimulation\\images\\Human_ABSENT.jpg"));
		humanPresenceR4ToggleButton.setBounds(1005, 357, 46, 47);
		frame.getContentPane().add(humanPresenceR4ToggleButton);
	}
}
